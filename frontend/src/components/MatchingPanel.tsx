        ...
        return found
          ? {
              ...
              y:
                height -
                padding -
                (found.value / maxVal) * (height - padding * 2),
            }
          : null;
      })
      ...
      >
        {Array.from({ length: ticks + 1 }).map((_, i) => (
          <line
            ...
            stroke="var(--color-chart-grid, rgba(255,255,255,0.06))"
          />
        ))}
        <line
          x1={padding}
          ...
          x2={width - padding}
          y2={height - padding}
          stroke="var(--color-chart-axis, rgba(255,255,255,0.2))"
        />
        <line
          x1={padding}
          y1={padding}
        ...
        {labels.map((l, i) => (
          <text
            ...
            fontSize="9"
            textAnchor="middle"
            fill="var(--color-chart-text, rgba(255,255,255,0.5))"
          >
            {l}
          </text>
        ))}
        {Array.from({ length: ticks + 1 }).map((_, i) => (
          <text
            key={i}
            x={padding - 6}
            y={height - padding - i * stepY + 3}
            fontSize="9"
            textAnchor="end"
            fill="var(--color-chart-text, rgba(255,255,255,0.5))"
          >
            {((maxVal / ticks) * i).toFixed(maxVal < 1 ? 4 : 0)}
          </text>
        ))}
        {yLabel && (
          <text
...
function MatchingPanel() {
  ...
  // Tab
  const [activeTab, setActiveTab] = useState<"validation" | "rapport">(
    "validation",
  );
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[var(--color-text-muted)] mt-1">
            Mise en correspondance automatique des produits fournisseurs au
            référentiel.
          </p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          ...
          <div className="card p-4 text-center">
            <div className="text-2xl font-bold text-[var(--color-text-muted)]">
              {stats.last_run?.ran_at
                ? new Date(stats.last_run.ran_at).toLocaleDateString("fr-FR", {
                    day: "2-digit",
                    month: "2-digit",
                    hour: "2-digit",
                    minute: "2-digit",
...
function RapportTab({
  ...
  runs: MatchingRunItem[];
  loading: boolean;
  onRefresh: () => void;
  stats: MatchingStatsData | null;
}) {
  ...
  const dateLabels = sorted.map((r) =>
    r.ran_at
      ? new Date(r.ran_at).toLocaleDateString("fr-FR", {
          day: "2-digit",
          month: "2-digit",
        })
      : "?",
  ...
  // Chart 1: Cost & Cache
  const costData: ChartPoint[] = sorted.map((r, i) => ({
    label: dateLabels[i],
    value: r.cost_estimate ?? 0,
  }));
  ...
  const stackedSeries = [
    {
      name: "Auto-matchés",
      color: "#22c55e",
      values: sorted.map((r) => r.auto_matched ?? 0),
    },
    {
      name: "À valider",
      color: "#f59e0b",
      values: sorted.map((r) => r.pending_review ?? 0),
    },
    {
      name: "Rejetés",
      color: "#ef4444",
      values: sorted.map((r) => r.auto_rejected ?? 0),
    },
    {
      name: "Non trouvés",
      color: "#6b7280",
      values: sorted.map((r) => r.not_found ?? 0),
    },
  ];

  // Chart 3: Products processed
  const productsData: ChartPoint[] = sorted.map((r, i) => ({
    label: dateLabels[i],
    value: r.total_products ?? 0,
  }));
  const autoData: ChartPoint[] = sorted.map((r, i) => ({
    label: dateLabels[i],
    value: r.auto_matched ?? 0,
  }));
        ...
        <>
          ...
          <div className="card p-4 space-y-2">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div>
                ...
                <MultiLineChart
                  series={[{ name: "Coût (€)", data: costData }]}
                  yLabel="€"
                />
              </div>
              <div>
                ...
                <MultiLineChart
                  series={[{ name: "Cache %", data: cacheData }]}
                  yLabel="%"
                />
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              {coveragePct !== null && stats ? (
                <>
                  ...
                  <p className="text-xs text-[var(--color-text-muted)]">
                    produits dans TCP / référentiel Odoo
                  </p>
                </>
              ) : (
                <p className="text-xs text-[var(--color-text-muted)]">
            ...
            <div className="card p-4 space-y-2 lg:col-span-2">
              <h4 className="text-xs font-semibold uppercase tracking-wide text-[var(--color-text-muted)]">
                Produits traités par run
              </h4>
              <MultiLineChart
                series={[
                  { name: "Produits traités", data: productsData },
                  { name: "Auto-matchés", data: autoData },
                ]}
              />
            </div>
          </div>
                ...
                <thead>
                  <tr className="border-b border-[var(--color-border-subtle)]">
                    {[
                      ...
                      <th
                        key={h}
                        className="px-3 py-2 text-left font-medium text-[var(--color-text-muted)] whitespace-nowrap"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody className="divide-y divide-[var(--color-border-subtle)]">
                  {[...sorted].reverse().map((r) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 whitespace-nowrap text-[#B8860B]">
                        {r.from_cache ?? "—"}
                      </td>
                      <td className="px-3 py-2 whitespace-nowrap text-emerald-400">
                        {r.auto_matched ?? "—"}
                      </td>
                      <td className="px-3 py-2 whitespace-nowrap text-amber-400">
                        {r.pending_review ?? "—"}
                      </td>
