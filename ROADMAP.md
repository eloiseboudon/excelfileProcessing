...
- **Disqualification variante modele** : les suffixes de gamme (Pro, Plus, Pro+, Ultra, Max, Lite, FE, FE+, Mini) et le suffixe S (14S vs 14 Pro) sont extraits et compares. Si les variantes different entre le label et le produit, le match est disqualifie (score 0). Empeche les faux positifs iPhone 14 Pro ↔ iPhone 14 Plus, Redmi Note 14 Pro ↔ Pro Plus, etc.
- **Stripping couleur dans le modele** : les noms de couleur (issus des tables colors + color_translations) sont retires du champ model avant le fuzzy matching. Corrige le scoring penalise quand la couleur est embeddee dans le nom produit (22% du referentiel affecte)
...
- **Feature flags** : `MATCHING_V2_ENABLED` (BM25), `MATCHING_EMBEDDINGS_ENABLED` (FAISS + cross-encoder). Rollback instantane
