...
class TestGetConfig:
    def test_returns_default_config(self, client, admin_headers):
        rv = client.get("/api/nightly/config", headers=admin_headers)
...
class TestTrigger:
    ...
    def test_trigger_returns_202(self, mock_thread, client, admin_headers):
        ...
        rv = client.post("/api/nightly/trigger", headers=admin_headers)
...
class TestJobs:
    def test_empty_list(self, client, admin_headers):
        rv = client.get("/api/nightly/jobs", headers=admin_headers)
        assert rv.status_code == 200
        assert rv.get_json() == []
    ...
    def test_get_single_job(self, client, admin_headers):
        ...
        rv = client.get(f"/api/nightly/jobs/{job.id}", headers=admin_headers)
        assert rv.status_code == 200
        assert rv.get_json()["id"] == job.id

    def test_get_unknown_job_404(self, client, admin_headers):
        rv = client.get("/api/nightly/jobs/9999", headers=admin_headers)
...
class TestRecipients:
    def test_list_empty(self, client, admin_headers):
        rv = client.get("/api/nightly/recipients", headers=admin_headers)
        assert rv.status_code == 200
        assert rv.get_json() == []

    def test_add_recipient(self, client, admin_headers):
        rv = client.post(
            "/api/nightly/recipients",
            headers=admin_headers,
    ...
    def test_delete_recipient(self, client, admin_headers):
        ...
        rv = client.delete(
            f"/api/nightly/recipients/{r.id}", headers=admin_headers
        )
        assert rv.status_code == 204
        assert NightlyEmailRecipient.query.get(r.id) is None

    def test_delete_unknown_404(self, client, admin_headers):
        rv = client.delete("/api/nightly/recipients/9999", headers=admin_headers)
        assert rv.status_code == 404
