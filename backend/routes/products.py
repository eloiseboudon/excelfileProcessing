    ...
    result = [
        {
            ...
            'price': c.price,
            'memory': c.product.memory.memory if c.product and c.product.memory else None,
            ...
            'week': 'S' + c.date.strftime('%W') + '-' + c.date.strftime('%Y') if c.date else None,
        }
    ...
    result = [
        {
            ...
            'reference': {'id': p.reference.id if p.reference else None, 'description': p.reference.description if p.reference else None} if p.reference else None,
        }
        for p in products
    ]
    return jsonify(result)
